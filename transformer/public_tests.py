...
# Description: Tests for the Transformer model implementation
# Notes:
#############################################################################################################
import numpy as np
...
def positional_encoding_test(target, get_angles):
    ...
    assert np.allclose(angs[0], trueAngs), "Wrong positional encoding"
...
def DecoderLayer_test(target, create_look_ahead_mask):
    ...
    out, attn_w_b1, attn_w_b2 = decoderLayerq(q, encoderq_output, True, look_ahead_mask, padding_mask)
    assert np.allclose(out[0, 0], [0.14950314, -1.6444231, 1.0268553, 0.4680646]), "Wrong values after masking?"
